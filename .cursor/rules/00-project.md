...
- Then skim `.cursor/rules/*.md` (this file plus style/testing rules) for anything relevant to the task at hand.
- Before commit/push: follow `.cursor/skills/commit-push/SKILL.md` (staged-only unless the user asked to stage; SemVer bump must be in staged `package.json`).
- Before release/tag/publish: follow `.cursor/skills/release/SKILL.md` (quality gates, annotated `v*` tag, push tags; npm publish only if explicitly requested).
- Before you do any work, mention how you could verify that work. Make sure you used best practices, were efficient, and didn't introduce any issues.
...
- Periodically (and when rules start feeling “noisy”): **self-audit** `.cursor/rules/*.md`, each `.cursor/skills/*/SKILL.md`, and `.cursor/HISTORY.md`, and remove/condense anything that is no longer needed, contradictory, duplicative, or bloating effectiveness. Keep guidance crisp and enforceable.
